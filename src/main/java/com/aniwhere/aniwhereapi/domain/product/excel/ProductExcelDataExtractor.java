...
import com.aniwhere.aniwhereapi.util.excel.ExcelDataExtractor;
        ...
        return new ExcelDataExtractor<>() {
            ...
            protected ProductDTO map(Row row) {
                ProductDTO dto = ProductDTO.builder()
                        ...
                        .story(row.getCell(3).getStringCellValue().trim())
                        .mdPick(row.getCell(4).getStringCellValue().trim().equals("Y") ? ProductMdPick.Y : ProductMdPick.N)
                        .adult(row.getCell(5).getStringCellValue().trim().equals("Y") ? Adult.Y : Adult.N)
                        .totalEpisode((int) row.getCell(6).getNumericCellValue())
                        .tagStrList(Arrays.asList(row.getCell(7).getStringCellValue().split(",")))
                        .releaseDate(dataFormatter.formatCellValue(row.getCell(8)))
                        .manufacturer(row.getCell(9).getStringCellValue().trim())
